...
class Detector4012LocalContourMaskTests(unittest.TestCase):
    ...
    def test_area_limits_remove_small_and_large_contours(self):
        binary = np.full((64, 64), 255, dtype=np.uint8)
        small = np.array([[[1, 1]], [[5, 1]], [[5, 5]], [[1, 5]]], dtype=np.int32)
        target = np.array([[[10, 10]], [[20, 10]], [[20, 20]], [[10, 20]]], dtype=np.int32)
        large = np.array([[[30, 30]], [[50, 30]], [[50, 50]], [[30, 50]]], dtype=np.int32)
        detector = Detector401_2(
            params={
                "min_area": 50,
                "max_area": 200,
        ...
        with (
            ...
            patch(
                "detectors.detector_401_2.cv2.findContours",
                return_value=([small, target, large], None),
            ),
        ):
            defects = detector.detect(np.zeros((64, 64), dtype=np.uint8))

        self.assertEqual(len(defects), 1)
        self.assertEqual(defects[0]["bbox_local"], [10, 10, 11, 11])
        self.assertEqual(defects[0]["area"], 100.0)
        self.assertEqual(defects[0]["metadata"]["min_area"], 50.0)
        self.assertEqual(defects[0]["metadata"]["max_area"], 200.0)
