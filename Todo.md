...
- [x] 2026-07-21：修正 401-2 偵測契約為每張 tile／有效 ROI 僅計算一次整體白像素比例，達門檻最多回傳一筆 NG；移除逐 contour 計數及不再適用的 `contour_mode`、`min_area`、`max_area` 配方／GUI schema，配方升版至 0.2.0，並新增離散白點、門檻、ROI bbox、metadata、CPU/CUDA fallback 與 production golden 回歸驗證。
- [x] 2026-07-21：稽核 401-2 CUDA fused/native 路徑，確認 GPU 僅產生與 CPU 等價的 binary mask，整體 tile 白點比例與單筆 NG 決策由共用 detector 後處理執行；新增 fused GPU 離散白點回歸測試，固定一次 GPU preprocess、整張 ROI 統計與最多一筆缺陷契約。當前機器無 `nvidia-smi`、`nvcc`、MSVC `cl`，未宣稱重建 DLL 或完成 RTX 3090 runtime 驗證。
- [x] 2026-07-21：更正 401-2 回復範圍：保留每張 tile／有效 ROI 僅計算一次整體白像素比例，僅將既有 `0.625` 門檻定義明文化為 `0～1` 比例的 `62.5%`，且 `>= 0.625` 為 NG、低於門檻為 PASS；新增精確邊界回歸測試，CPU/CUDA 共用同一判定。
